...
echo "-----> Running manage.py check --deploy --fail-level CRITICAL"
if [ -n "$DATABASE_URL" ]; then
    poetry run backend/manage.py check --deploy --fail-level CRITICAL
else
...
if [ -n "${SENTRY_API_KEY:-}" ] && [ -n "${SENTRY_ORG:-}" ] && [ -n "${SENTRY_PROJECT_NAME:-}" ] && [ -n "${RENDER_GIT_COMMIT:-}" ]; then
    ...
    echo "-----> Skipping Sentry source maps (no SENTRY_API_KEY)"
fi
